...
class Design extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    var firstendpoint = Offset(size.width - 140, size.height / 2 - 6);
    var firstcontrolpoint = Offset(size.width - 70, size.height / 2 + 4);

    path.quadraticBezierTo(firstcontrolpoint.dx, firstcontrolpoint.dy,
        firstendpoint.dx, firstendpoint.dy);

    var secondendpoint = Offset(size.width * 0.25, 0.0);
    var secondcontrolpoint = Offset(size.width - 210, size.height / 2 - 24);
